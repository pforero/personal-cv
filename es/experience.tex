%% cSpell: language en,es
%-------------------------------------------------------------------------------
%	SECTION TITLE
%-------------------------------------------------------------------------------
\cvsection{Experiencia Profesional}


%-------------------------------------------------------------------------------
%	CONTENT
%-------------------------------------------------------------------------------
\begin{cventries}

%---------------------------------------------------------
\cventry
    {Científico de Datos, Innovation Lab} % Job title
    {Allfunds} % Organization
    {Madrid, España} % Location
    {2018 - Actualidad} % Date(s)
    {
        \begin{cvitems} % Description(s) of tasks/responsibilities
            \item {Desarrollo de un modelo predictivo usando \href{https://es.wikipedia.org/wiki/Random_forest}{Random Forests} para crear perfiles de preferencias de compra de fondos de inversión para clientes minoristas.}
            \item {Iniciación de un proyecto experimental para clasificar fondos de inversión utilizando \href{https://es.wikipedia.org/wiki/Procesamiento_de_lenguajes_naturales}{Procesamiento de Lenguajes Naturales} con prospectos en formato PDF.}
            \item {Definición y análisis de un proyecto para implementar un CRM en la empresa.}
            \item {Asistencia en el desarrollo de un \textit{Datalake} para la empresa usando Google Cloud.}
            \item {Creación de un herramienta Python para el análisis sistemático de precios.}
            \item {Desarrollo de la segmentación de información interna y datos analíticos para informes de gestión.}
            \item {Ayuda en el desarrollo de una nueva herramienta de calculo de \textit{rebates} en la nube.}
            \item {Creación de un modulo para la generación en nube de informes de precios para usuarios internos y externos.}
        \end{cvitems}
    }

%---------------------------------------------------------
\cventry
    {Profesor Ayudante} % Job title
    {SAIS John Hopkins University} % Organization
    {Bolonia, Italia} % Location
    {2015} % Date(s)
    {
        \begin{cvitems} % Description(s) of tasks/responsibilities
            \item {Profesor ayudante en Comercio Internacional para el Professor G. Plummer.}
        \end{cvitems}
    }

%---------------------------------------------------------
\cventry
    {Analista, Gestión de Activos} % Job title
    {Banif (Santander Banca Privada)} % Organization
    {Madrid, España} % Location
    {2009 - 2010} % Date(s)
    {
        \begin{cvitems} % Description(s) of tasks/responsibilities
            \item {Análisis de activos y portfolios para los gestores.}
            \item {Generación de los informes semanales de las carteras de renta fija.}
        \end{cvitems}
    }

%---------------------------------------------------------
\cventry
    {Practicas, Analista} % Job title
    {Renaissance Capital} % Organization
    {Londres, Reino Unido} % Location
    {2008} % Date(s)
    {
        \begin{cvitems} % Description(s) of tasks/responsibilities
            \item {Análisis de mercados emergentes sub-saharianos.}
        \end{cvitems}
    }

%---------------------------------------------------------
\cventry
    {Practicas, Analista} % Job title
    {CQS} % Organization
    {Londres, Reino Unido} % Location
    {2008} % Date(s)
    {
        \begin{cvitems} % Description(s) of tasks/responsibilities
            \item {Apoyo a gestores de carteras en el análisis de empresas, sobre todo en las obligaciones de sus pensiones.}
        \end{cvitems}
    }

%---------------------------------------------------------
\cventry
    {Practicas, Analista} % Job title
    {ING Capital} % Organization
    {Londres, Reino Unido} % Location
    {2007} % Date(s)
    {
        \begin{cvitems} % Description(s) of tasks/responsibilities
            \item {Análisis de países emergentes en Europea del este y utilities en Iberia.}
        \end{cvitems}
    }

%---------------------------------------------------------
\end{cventries}

%% cSpell: ignore Bolonia Londres prospectos